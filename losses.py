...
####################     For multi layer perceptual loss
#class perceptual_loss(nn.Module):
#    def __init__(self, vgg):
#        super(perceptual_loss, self).__init__()
#        self.normalization_mean = [0.485, 0.456, 0.406]
...
#        self.criterion = nn.MSELoss()
#
#    def forward(self, HR, SR, layers=None):
#        if layers is None:
#            layers = ['conv5_4', 'conv2_1']
#        
#        ## HR and SR should be normalized [0,1]
#        hr = self.transform(HR)
#        sr = self.transform(SR)
...
#        sr_feat = {}
#        for layer in layers:
#            hr_feat[layer] = getattr(self.vgg(hr), layer)
#            sr_feat[layer] = getattr(self.vgg(sr), layer)
#        
#        loss = sum(self.criterion(hr_feat[layer], sr_feat[layer]) for layer in layers)
#        
#        return loss, hr_feat, sr_feat
